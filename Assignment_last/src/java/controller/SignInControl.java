...
public class SignInControl extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Cookie cuser = new Cookie("cuser", txt);
        ...
        for (Account account : list) {
            if (account.getEmailById() != null) {
                if ((txt.equals(account.getUsername()) || (txt.equals(account.getEmailById()))) && (password.equals(account.getPassword()))) {
                    HttpSession session = request.getSession();
                    session.setAttribute("role", account.getRole());
                    session.setAttribute("aid", account.getId());
                    session.setMaxInactiveInterval(60 * 60 * 60 * 60);
                    response.sendRedirect("home");
                    tmp = 1;
                }
            }else{
                if(txt.equals(account.getUsername())&& password.equals(account.getPassword())){
                    HttpSession session = request.getSession();
                    session.setAttribute("role", account.getRole());
                    session.setAttribute("aid", account.getId());
                    session.setMaxInactiveInterval(60 * 60 * 60 * 60);
